...
//Struct per retornar les 3 direccions del Gskew
/*
typedef struct Dirs{
 unsigned char *dir1;
 unsigned char *dir2;
...
/*    if (!l1size || (l1size & (l1size-1)) != 0)
  fatal("level-1 size, `%d', must be non-zero and a power of two", 
        l1size);*/
      // tamany l1 = 1 sempre
      pred_dir->config.gskew.l1size = 1;
  ...
  fatal("shift register width, `%d', must be non-zero and positive",
      ...
      // xor no importa, per tant = 1
      pred_dir->config.gskew.xor = 1;
      pred_dir->config.gskew.shiftregs = calloc(l1size, sizeof(int));
      if (!pred_dir->config.gskew.shiftregs)
  fatal("cannot allocate shift register table");
      
      pred_dir->config.gskew.l2table = calloc(l2size, sizeof(unsigned char));
      if (!pred_dir->config.gskew.l2table)
  fatal("cannot allocate second level table 1");

pred_dir->config.gskew.l2table2 = calloc(l2size, sizeof(unsigned char));
      if (!pred_dir->config.gskew.l2table2)
  fatal("cannot allocate second level table 2");

pred_dir->config.gskew.l2table3 = calloc(l2size, sizeof(unsigned char));
      if (!pred_dir->config.gskew.l2table3)
  fatal("cannot allocate second level table 3");
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->class) {
    ...
    case BPredGskew:
    {
      int l2index1, l2index2,l2index3, l1index;
      
      l1index = (baddr >> MD_BR_SHIFT) & (pred_dir->config.gskew.l1size - 1);
      l2index1 = pred_dir->config.gskew.shiftregs[l1index];
      l2index2 = pred_dir->config.gskew.shiftregs[l1index];

      //struct Dirs *direccions = malloc(sizeof(struct Dirs));


      //xor
      
      l2index1 = ~(((l2index1 ^ (baddr >> MD_BR_SHIFT)) & ((1 << pred_dir->config.gskew.shift_width) - 1)) | ((baddr >> MD_BR_SHIFT)
         << pred_dir->config.gskew.shift_width));

      l2index1 = l2index1 & (pred_dir->config.gskew.l2size - 1);
      

       //concatenat1

      l2index2 = l2index2 | ((baddr >> MD_BR_SHIFT) << pred_dir->config.gskew.shift_width);
      l2index2 = l2index2 & (pred_dir->config.gskew.l2size - 1);

      //concatenat2

      l2index3 = l2index3 | (pred_dir->config.gskew.shift_width << (baddr >> MD_BR_SHIFT));
      l2index3 = l2index3 & (pred_dir->config.gskew.l2size - 1);

      //Actualitzem direccions de les taules
      /*
      direccions->dir1 = &pred_dir->config.gskew.l2table[l2index1];
      direccions->dir2 = &pred_dir->config.gskew.l2table2[l2index2];
      direccions->dir3 = &pred_dir->config.gskew.l2table3[l2index3];
      */

      p = &pred_dir->config.gskew.l2table[l2index1];
      p2 = &pred_dir->config.gskew.l2table2[l2index2];
      p3 = &pred_dir->config.gskew.l2table3[l2index3];
      /*
      fprintf(stderr, "p1:%d\n", p);
      fprintf(stderr, "p2:%d\n", p2);
      fprintf(stderr, "p3:%d\n", p3);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////      
    ...
    {
      /*
      struct Dirs *direccions; 
      direccions = bpred_dir_lookup (pred->dirpred.gskew, baddr);
      fprintf(stderr, "Dirs: %d\n", direccions);
      
      dir_update_ptr->pdir2 = direccions->dir1;
      dir_update_ptr->pdir2_2 = direccions->dir2;
      dir_update_ptr->pdir2_3 = direccions->dir3;
      */

      dir_update_ptr->pdir1 = bpred_dir_lookup (pred->dirpred.gskew, baddr);
      dir_update_ptr->pdir2 = p2;
      dir_update_ptr->pdir3 = p3;

      /*
      fprintf(stderr, "Dir1:%d\n", direccions->dir1);
      fprintf(stderr, "Dir1:%d\n", direccions->dir2);
      fprintf(stderr, "Dir1:%d\n", direccions->dir3);

      fprintf(stderr, "UP:%d\n", dir_update_ptr->pdir2);
      fprintf(stderr, "UP:%d\n", dir_update_ptr->pdir2_2);
      fprintf(stderr, "UP:%d\n", dir_update_ptr->pdir2_3);
      */
    }
      break;
//////////////////////////////////////////////////////////////////////////////////////////////////////////////      
...
if (!acierta || pred->class != BPredGskew ){
  if(!taken || pred->class != BPredGskew){
  ...
  { /* not taken */
    ...
    /* Gskew predictor also updates third predictor and meta predictor */
  /* second direction predictor */
...
/* update state (but not for jumps) */
  if (dir_update_ptr->pdir1 && (!!taken == !!(*dir_update_ptr->pdir1) & 2))
    {
      if (taken)
  {
    if (*dir_update_ptr->pdir1 < 3)
  ...
  /* combining predictor also updates second predictor and meta predictor */
  /* second direction predictor */
  if (dir_update_ptr->pdir2 && (!!taken == (*dir_update_ptr->pdir2) & 2))
    {
      if (taken)
  {
    if (*dir_update_ptr->pdir2 < 3)
  ...
  { /* not taken */
    ...
    /* Gskew predictor also updates third predictor and meta predictor */
  /* second direction predictor */
  if (dir_update_ptr->pdir3 && (!!taken == !!(*dir_update_ptr->pdir3) & 2))
    {
      if (taken)
  {
    if (*dir_update_ptr->pdir3 < 3)
